
import { HiddenMail } from '@/components/HiddenMail.jsx';

const FAQ = [{
  pregunta: '¿En que consisten los talleres?',
  respuesta: <>Los talleres son el corazón del Encuentro. Son espacios abiertos, participativos y horizontales donde mujeres, lesbianas, travestis, trans, bisexuales, intersexuales y no binaries compartimos experiencias, debates y propuestas sobre las problemáticas que atraviesamos en nuestras vidas y territorios.
Cada taller aborda una temática específica y tiene una dinámica que garantiza que todas las voces sean escuchadas. Al final de los talleres se sintetizan todos los aportes, acuerdos y diferencias.   </>
}, {
  pregunta: '¿Quiénes pueden participar?',
  respuesta: <>Todas las personas que formen del Encuentro. No es necesario inscribirse previamente en un taller. Podés participar de los talleres que te interesen durante cada jornada. Consultá el cronograma para conocer días, horarios y ubicaciones</>
}, {
  pregunta: '¿Donde consulto el cronograma?',
  respuesta: <>El cronograma completo del Encuentro estará disponible en esta página web y en nuestras redes oficiales. Allí vas a encontrar: Horarios de apertura y acreditación, Cronograma de talleres, Actividades culturales, Feria, festivales y eventos especiales, Información sobre la marcha y actos de apertura y cierre del encuentro. <br/>
    Guardá esta página y volvé a visitarla cerca de la fecha del Encuentro para acceder a la versión actualizada.</>
}, {
  pregunta: '¿Cómo se financia el Encuentro?',
  respuesta: <>El Encuentro es autogestivo, autónomo y autofinanciado. Su organización es posible gracias al trabajo voluntario de cientos de compañeras y compañeres, los aportes solidarios de organizaciones, actividades de recaudación y contribuciones realizadas por quienes participan. Esta forma de organización garantiza la independencia política del Encuentro y fortalece una construcción colectiva sostenida por la participación de miles de personas de todo el país. Si querés colaborar con la organización, podés sumarte a las comisiones de trabajo o realizar un aporte solidario. </>
}, {
  pregunta: '¿Dónde consulto para alojarme gratuitamente?',
  respuesta: <>La Comisión de Alojamiento coordina espacios de hospedaje solidario para quienes viajan desde otras localidades, provincias o barrios alejados del casco céntrico y necesitan alojamiento durante los días del Encuentro. Si estás en una organización, colectiva, grupalidad o viajás sola o sole, inscribirte para solicitar alojamiento, comunicate este mail <strong><HiddenMail mail="alojamiento.39encuentropluri.cba@proton.me" /></strong> <br/>
  Por ese medio te especificaremos qué información necesitamos y cómo compartirla de manera más segura.</>
}, {
  pregunta: '¿Cómo será el transporte los días del Encuentro?',
  respuesta: <>Si venís a participar del 39° Encuentro Plurinacional de Mujeres, Lesbianas, Travestis, Trans, Bisexuales, Intersexuales y No Binaries, queremos que puedas trasladarte de manera sencilla y segura entre las distintas sedes y actividades.
    {/* Transporte urbano gratuito */}
    {/* Durante los días del Encuentro, el transporte urbano de pasajeros de la ciudad de Córdoba será gratuito para quienes participen de las actividades. De esta manera se busca garantizar el acceso a los talleres, actividades culturales, espacios de intercambio y demás propuestas que se desarrollarán en distintos puntos de la ciudad. */}
    Si llegas a la ciudad de Córdoba <br/>
    <ul className="ml-6 mt-2 list-disc space-y-2">
    <li>Terminal 1 (Bv. Juan Domingo Perón 380): desde aquí parten y llegan la mayoría de los servicios de larga distancia e interprovinciales.<br/></li>
    <li>Terminal 2 o Terminal del Bicentenario (Bv. Juan Domingo Perón 660): concentra principalmente servicios hacia distintos destinos de la provincia de Córdoba y se encuentra conectada con la Terminal 1.<br/></li>
    </ul>
    Ambas terminales se encuentran a pocas cuadras del centro de la ciudad, con conexión a numerosas líneas del transporte urbano.<br/>
    Aeropuerto Internacional Ingeniero Ambrosio Taravella: opción de transporte urbano "Aerobús" para llegar al centro de la ciudad.<br/>
    En la <a href="/Transporte" className="text-blue-500 hover:underline">página de Transporte</a>  de esta web vas a encontrar información actualizada sobre:<br/>
    <ul className="ml-6 mt-2 list-disc space-y-2">
    <li>Líneas de colectivo y recorridos.</li>
    <li>Aplicaciones para consultar horarios y recorridos en tiempo real.</li>
    <li>Recomendaciones para moverte durante el Encuentro.</li>
    <li>Información sobre accesibilidad.</li>
    </ul>
    Para aprovechar todas las actividades del Encuentro.
    {/* Visitá la <a href="/Transporte" className="text-blue-500 hover:underline">página de Transporte</a> para aprovechar todas las actividades del Encuentro.<br/> */}
  </>
},
];
export default FAQ;